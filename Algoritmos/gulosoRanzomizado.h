...
// funcao que verifica se tais vetores formam ciclo
...
//chamada recursiva funcao aciclico
...
Grafo* faseUm(Grafo *grafo, float alfa, int numIter, int d) {
	int i = 1, k;
	//Solucao s e solBest
	Grafo *s = new Grafo(0, false, true, false);
	Grafo *solBest = NULL;
	...
	while(i<numIter){
		...
		// percorre a lista de nohs do grafo, calculando o peso e setando a lista de d nohs adjacentes de cada noh
		...
		while(!nosNaoVisitados.empty()) {
			...
			cout << "É ali mesmo" << endl;
			k = randomRange(0, alfa * (nosNaoVisitados.size()-1));
			cout << "Passei garai" << endl;
			// guarda o primeiro noh na lista nosVisitados que faz parte dos dNosAdjacentes do noMenorPeso
			list<No*>::iterator noAux;
			int iterator = 0;
			No* noPrimeiro;
			for(noAux = nosNaoVisitados.begin(); noAux != nosNaoVisitados.end(); noAux++){
				if(iterator == k){
					noPrimeiro = (*noAux);
				}
				iterator++;
			...
			for(noAux = dNosAdjacentes[noPrimeiro].begin(); noAux != dNosAdjacentes[noPrimeiro].end(); noAux++) {
			...
			if(flag1 == 1) {
				...
				// insere o noPrimeiro na lista nosVisitados, remove da nosNaovisitados
				nosVisitados.push_back(noPrimeiro);
				nosNaoVisitados.remove(noPrimeiro);

				// insere o noPrimeiro na arvore
				if(s->getNo(noPrimeiro->getId()) == nullptr)
					s->inserirNo(noPrimeiro->getId());

				// insere o noAux na s
				if(s->getNo((*noAux)->getId()) == nullptr)
					s->inserirNo((*noAux)->getId());

				// insere um aresta entre eles, com o peso da aresta entre ele no grafo
				s->inserirAresta(noPrimeiro->getId(), (*noAux)->getId(), noPrimeiro->getAresta((*noAux)->getId())->getPeso());

				// aumenta o grau dos dois nohs
				noPrimeiro->setGrau(noPrimeiro->getGrau()+1);
				...
				// insere o noPrimeiro na lista nosVisitados, remove da nosNaovisitados
				nosNaoVisitados.remove(noPrimeiro);
				nosEstadoEspera.push_back(noPrimeiro);
		...
		// a funcao insereRestanteNos eh chamada para inserir os nohs na s
		if(count > 0)
			insereRestanteNos(nosEstadoEspera, nosVisitados, s, d);
		if(solBest == NULL){
			solBest = s;
		}else{
			int custo_solBest = calculaPeso(solBest);
			int custo_s = calculaPeso(s);
			if(custo_s < custo_solBest){
				solBest = s;
			}
		}
	}
...
Grafo* gulosoRandomizado(Grafo *grafo, int d) {
	Grafo *arvore = new Grafo(0, false, true, true);
	arvore = faseUm(grafo, 0.1, 3, d);
	arvore->criaListaAdjacencia();	
